...
寄存器地址为 16 位，访问方式为先写 2 字节地址（高字节在前）再读数据。地址与含义取自厂商触摸驱动 `1-示例程序_Demo/Arduino/Install libraries/ST77922_TOUCH/ST77922_Touch.cpp`（头文件 `ST77922_Touch.h`）与 IDF 侧 `1-示例程序_Demo/ESP-IDF/3.5inch_ESP32-S3_LVGL/components/esp_bsp/bsp_touch.c`。[文档]

| 地址 | 名称 | 含义 |
| --- | --- | --- |
| `0x0000` | `FW_VER` | 固件版本 |
| `0x0001` | `STATUS` | 状态；低 4 位为 0 表示就绪 |
| `0x0002` | `DEVICE_CON` | 设备配置 |
| `0x0005` | `MAX_X` | X 轴坐标上限 |
| `0x0007` | `MAX_Y` | Y 轴坐标上限 |
| `0x0009` | `MAX_TOUCHES` | 触点数上限 |
| `0x0010` | `TOUCH_INFO` | 触摸信息；bit3 置位表示触点数据已更新 |
| `0x0012` | `GESTURE_INFO` | 手势信息 |
| `0x0014 + N × 7` | 触点 N | 第 N 个触点的 7 字节帧，N 自 0 起 |

从地址与触点数上限由控制器应答取得：复位时序（RST 拉低 100 ms 后拉高 100 ms）完成后轮询 `0x0001`，低 4 位清零表示就绪；随后读 `0x0009`，返回 5。触点数据在 `0x0010` 的 bit3 置位后才读取。

触点点帧为 7 字节：`[0]` 最高位表示该点有效、低 6 位为 X 高 6 位，`[1]` 为 X 低 8 位，`[2]` 低 6 位为 Y 高 6 位，`[3]` 为 Y 低 8 位，`[4..6]` 未使用。
...
| 信号 | GPIO | 说明 | 来源 |
| --- | --- | --- | --- |
| 扩展 IO | IO45 | 引出至 1.25 mm 4P 座。同时为启动配置引脚，见 2.9 | [文档] |
| 扩展 IO | IO46 | 同上。同时为启动配置引脚，见 2.9 | [文档] |
